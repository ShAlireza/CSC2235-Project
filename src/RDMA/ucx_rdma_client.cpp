...
static void send_cb(void *request, ucs_status_t status, void *user_data) {
    ...
    std::cout << "Client: AM message " << id << " sent (status = " 
              << ucs_status_string(status) << ")\n";
}

static void rdma_cb(void *request, ucs_status_t status, void *user_data) {
    int id = (int)(uintptr_t)user_data;
    std::cout << "Client: RDMA write " << id << " completed (status = " 
              << ucs_status_string(status) << ")\n";
}

static ucs_status_t rkey_recv_cb(void *arg, const void *header, size_t header_length,
...
UcxRdmaClient::UcxRdmaClient(const std::string &server_ip, size_t buffer_size, size_t chunk_size)
...
void UcxRdmaClient::send_metadata() {
    std::string msg = std::to_string(chunk_size) + " " + std::to_string(buffer_size);
    ucp_request_param_t param = {};
    param.op_attr_mask = UCP_OP_ATTR_FIELD_CALLBACK | UCP_OP_ATTR_FIELD_USER_DATA;
    param.user_data = (void *)(uintptr_t)0;
    param.cb.send = send_cb;
    void *req = ucp_am_send_nbx(ep, AM_ID, nullptr, 0, msg.c_str(), msg.size() + 1, &param);
...
void UcxRdmaClient::wait_for_rkey() {
    while (!rkey_received) ucp_worker_progress(worker);
}

void UcxRdmaClient::send_chunk(int *data, size_t size) {
    ucp_request_param_t put_param = {};
    put_param.op_attr_mask = UCP_OP_ATTR_FIELD_CALLBACK | UCP_OP_ATTR_FIELD_USER_DATA;
    put_param.user_data = (void *)(uintptr_t)(requests.size());
    put_param.cb.send = rdma_cb;
    void *req = ucp_put_nbx(ep, data, size, remote_addr + current_offset, rkey, &put_param);
